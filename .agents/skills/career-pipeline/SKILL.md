...
1. `profile build`로 후보 경험 원장을 만들고 사용자가 승인한 `experience_ledger.json`만 사용한다. 승인본은 `profile validate`로 검사한다. `profile validate`는 원장 스키마와 함께 `workspace_root`의 근거 파일 존재 여부, 원본 SHA-256, 근거 문단 SHA-256을 확인한다. `경험정리` 자료가 변경·이동·삭제된 상태에서 이전 원장을 재사용하지 않는다.
...
여섯 게이트가 모두 통과한 경우에만 `ready`다. 활성 공고는 유효하지만 나머지 검토가 남으면 `review_required`, 공고가 없거나 오래됐으면 `not_ready`다. 차단 사유는 `blocker_codes`와 설명으로 남긴다. 유튜브 작성전략은 `writing_guidance.freshness`로 원본 대비 최신성을 확인하되 계속 전략 자료로만 사용한다. `prepare --target`에서는 `writing_guidance.target_specific`으로 대상 기관과 일치하는 유튜브 사례를 별도 기록할 수 있지만, 해당 사례의 기관 사실·수치·업무 설명은 공식 조사 근거로 사용하지 않는다.
