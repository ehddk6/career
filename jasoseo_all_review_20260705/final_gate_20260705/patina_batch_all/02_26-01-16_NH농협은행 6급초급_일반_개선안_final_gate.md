{
  "mode": "score",
  "format": "json",
  "overall": 38.5,
  "categories": [
    {
      "name": "content",
      "weight": 0.18,
      "detected": null,
      "rawScore": 11.1,
      "weighted": 2
    },
    {
      "name": "language",
      "weight": 0.18,
      "detected": null,
      "rawScore": 25,
      "weighted": 4.5
    },
    {
      "name": "style",
      "weight": 0.18,
      "detected": null,
      "rawScore": 22.2,
      "weighted": 4
    },
    {
      "name": "communication",
      "weight": 0.13,
      "detected": null,
      "rawScore": 0,
      "weighted": 0
    },
    {
      "name": "filler",
      "weight": 0.08,
      "detected": null,
      "rawScore": 16.7,
      "weighted": 1.3
    },
    {
      "name": "structure",
      "weight": 0.15,
      "detected": null,
      "rawScore": 33.3,
      "weighted": 5
    },
    {
      "name": "viral-hook",
      "weight": 0.1,
      "detected": null,
      "rawScore": 0,
      "weighted": 0
    }
  ],
  "tone": {
    "tone": null,
    "tone_source": "profile_only",
    "tone_evidence": [],
    "tone_confidence": null
  },
  "mps": null,
  "gateResult": {
    "threshold": 30,
    "overall": 38.5,
    "passed": false,
    "exitCode": 3
  },
  "output": "| Category | Weight | Detected | Raw Score | Weighted |\n|----------|--------|----------|-----------|----------|\n| content | 0.18 | 1/6 | 11.1 | 2.0 |\n| language | 0.18 | 4/8 | 25.0 | 4.5 |\n| style | 0.18 | 2/6 | 22.2 | 4.0 |\n| communication | 0.13 | 0/4 | 0.0 | 0.0 |\n| filler | 0.08 | 1/4 | 16.7 | 1.3 |\n| structure | 0.15 | 2/5 | 33.3 | 5.0 |\n| viral-hook | 0.1 | 0/9 | 0.0 | 0.0 |\n| **Overall** | | | | **16.8 (±10)** |\n\n판정: **거의 사람다움**.  \n주요 신호는 문항마다 비슷한 구조가 반복되는 점, “NH농협은행 + 직무 키워드 + 경험 + 그 결과 + 입행 후 계획” 흐름이 거의 같은 방식으로 이어지는 점입니다. 바이럴 훅이나 챗봇 표현은 없습니다.\n\n---\ntone: null\ntone_source: profile_only\ntone_evidence: []\ntone_confidence: null\n---",
  "scores": {
    "llm": {
      "overall": 16.8,
      "interpretation": "mostly human"
    },
    "deterministic": {
      "overall": 38.5,
      "interpretation": "mixed",
      "skipped": false,
      "skipReason": null,
      "paragraphCount": 13,
      "hotParagraphs": 5,
      "signalScore": 24.5,
      "bands": {
        "burstiness": {
          "low": 5,
          "mid": 0,
          "high": 0,
          "null": 8
        },
        "mattr": {
          "low": 0,
          "mid": 0,
          "high": 12,
          "null": 1
        },
        "lexicon": {
          "hot": 0,
          "threshold": 3
        },
        "koDiagnostics": {
          "hot": 0,
          "thresholds": {
            "minSentences": 4,
            "minEojeols": 20,
            "spacing": {
              "maxEojeolLengthCV": 0.38
            },
            "comma": {
              "maxPerSentence": 1
            },
            "posProxy": {
              "minMatchedCount": 10,
              "maxClassDiversity": 0.26
            }
          }
        },
        "markupLeakage": {
          "leaked": false,
          "hits": 0,
          "floor": 90
        },
        "discourseTells": {
          "hot": false,
          "fakeCandor": {
            "count": 0,
            "hits": [],
            "hot": false,
            "threshold": 2
          },
          "thematicBreaks": {
            "count": 0,
            "adjacentToHeading": 0,
            "hot": false,
            "threshold": 3
          }
        },
        "structuralClassifier": {
          "available": false,
          "hot": null,
          "score": null,
          "floor": 0
        }
      }
    },
    "preference": {
      "reason": "deterministic-divergence",
      "selected": "deterministic",
      "threshold": 20,
      "llmOverall": 16.8,
      "deterministicOverall": 38.5,
      "overall": 38.5
    }
  }
}