...
def _kind(text:str,support:Sequence[AuthorityRecord])->str:
    ...
    kinds={r.source_kind for r in support}
    return "applicant_fact" if kinds=={"applicant"} else "research_fact" if kinds=={"research"} else "synthesis"
def compile_response_assertions(question_index:int,answer:str,context:AuthorityContext)->list[dict[str,Any]]:
    ...
    for si,sentence in enumerate(sentences,1):
        for ci,clause in enumerate(_clauses(sentence),1):
            ...
            if causal and any(c in clause for c in _OWN) and any(str(r.metadata.get("contribution","unknown")) in {"observed","unknown"} for r in app) and any(v in clause for v in _CAUSE_VERBS):contradictions.append("ownership_overclaim_risk")
            status="unsupported" if (not support or unsupported) else "needs_review" if causal and not caused else "bounded_interpretation" if len(support)>1 and _kind(clause,support)=="synthesis" else "supported"
...
def write_assertion_artifacts(run_dir:Path,*,draft_path:Path|None=None):
    ...
    lines=["# 최종 주장 컴파일","",f"- 총 주장: {report['summary']['total']}",f"- unsupported: {report['summary']['unsupported']}",f"- needs_review: {report['summary']['needs_review']}",""]
